...
def process_image(image):
    ...
    # Invert the edges (black lines on white background)
    inverted_edges = cv2.bitwise_not(edges)

    # Create a white background image
    white_bg = np.ones_like(inverted_edges) * 255  # White background

    # Combine edges with the white background
    final_image = np.minimum(inverted_edges, white_bg)

    # Convert back to RGB format for displaying in Streamlit
    processed_image = cv2.cvtColor(final_image, cv2.COLOR_GRAY2RGB)
